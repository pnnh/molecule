...
int MailService::insertMail(const MailModel &model) {
    const char *sqlText =
            "insert into mails (pk, title, content, create_time, update_time, creator, sender, receiver) "
...
int MailService::updateMail(const MailModel &model) {
    const char *sqlText =
            "update mails set title = $1, content = $2, update_time = $3, sender = $4, receiver = $5 "
            "where pk = $6;";
