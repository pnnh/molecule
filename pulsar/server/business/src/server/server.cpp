...
void PulsarServer::read_request(const std::shared_ptr<http_connection> &connection) {

    boost::beast::http::async_read(
            connection->socket_, connection->buffer_, connection->request_,
            [connection, this](boost::beast::error_code ec, std::size_t bytes_transferred) {
...
void PulsarServer::process_request(const std::shared_ptr<http_connection> &connection) {
    ...
    switch (connection->request_.method()) {
        ...
        default:
            ...
            connection->response_.set(boost::beast::http::field::content_type, "text/plain");
            boost::beast::ostream(connection->response_.body())
                    << "Invalid request-method '" << std::string(connection->request_.method_string())
                    << "'";
            break;
    }
}

void PulsarServer::create_response(const std::shared_ptr<http_connection> &connection) {
    auto isMd5 = connection->request_.target().find("/md5", 0);
    auto url = "http://localhost" + std::string(connection->request_.target());
    auto isArticle = connection->request_.target().find("/blog/articles/", 0) == 0;
    ...
    if (isMd5 == 0) {
        ...
        if (handler != nullptr) {
            try {
                ...
                connection->response_.result(boost::beast::http::status::internal_server_error);
            }
        } else {
            connection->response_.result(boost::beast::http::status::not_found);
            connection->response_.set(boost::beast::http::field::content_type, "text/plain");
            boost::beast::ostream(connection->response_.body()) << "File not found\r\n";
        }
    }
    write_response(connection);
}

void PulsarServer::write_response(const std::shared_ptr<http_connection> &connection) {

    connection->response_.content_length(connection->response_.body().size());

    boost::beast::http::async_write(
            connection->socket_, connection->response_, [connection](boost::beast::error_code ec, std::size_t) {
                connection->socket_.shutdown(boost::asio::ip::tcp::socket::shutdown_send, ec);
