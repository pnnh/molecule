...
void WorkerThreadLoop(boost::shared_ptr<boost::asio::io_service> iosvc, int counter) {
...
void TimerHandler(const boost::system::error_code &ec) {
  if (ec) {
    ...
    global_stream_lock.lock();
    std::cout << "Your see thie line because you have waited for 10 seconds.\n";
...
int run_main() {
  ...
  for (int i = 1; i <= 5; i++) {
    threads.create_thread(boost::bind(&WorkerThreadLoop, io_svc, i));
  }
