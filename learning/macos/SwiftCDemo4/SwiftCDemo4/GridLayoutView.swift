...
//    @State private var data = ["c.gif", "d.jpg", "e.jpg", "f.png", "g.png", "zhe.jpg", "b.png"]
    @State private var data:[ImageModel2] = []
                            ...
                            if let paths = contentsOfPath {
                                ...
                                for fileName in paths {
                                    ...
                                    if fileName.hasSuffix(".jpg") || fileName.hasSuffix(".png") || fileName.hasSuffix(".gif"){
                                        let imgModel = ImageModel2(Path: "\(path)/\(fileName)", Text: "待检查")
                                        self.data.append(
                                            imgModel
                                        )
                                    }
...
//                    Text(self.data[self.index].Text)
                    ...
                    Button(action: {
                        ...
                        let cStr = recognize(self.data[self.index].Path)
                        if let output = cStr  {
                            let imgText = String(cString: output)
                            print("输出 \(imgText )")
//                            self.imgText = imgText
                            self.data[self.index].Text = imgText
...
func getImagePath(data: [ImageModel2], index: Int) -> String {
...
func calcRow(data: [ImageModel2], columnCount: Int = 10) -> Int {
