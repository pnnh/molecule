...
export function Table (props: {
  ...
  const goPage = (page: number, pageSize: number) => {
    ...
    const redirectUrl = '/console/stage?' + queryString.stringify(queryObject)
...
function TableRow (props: { model: StageModel }) {
      ...
      <div className={styles.rowContent + ' row'}>
        ...
        <div className={'col-2 col-xl-2'}>
          <div className={styles.fieldItem}>
            <label>构建作业</label>
            <div className={styles.content}>
              <Link href={'/console/job?stage=' + props.model.id}>构建作业</Link>
            </div>
          </div>
        </div>
      </div>
...
function calcOperatorItems (model: StageModel): MenuProps {
