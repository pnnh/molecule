...
import {StageClient} from '@/services/client/stage'

export function AddButton () {
  const [show, setShow] = React.useState(false)
  const router = useRouter()

  const onFormFinish = async (values: StageModel) => {
    console.log('Success:', values)
    const result = await StageClient.insertModel(values)
  ...
  return <>
    ...
    <Modal title="新增阶段" footer={null} open={show} onOk={async () => {
      setShow(false)
    }} onCancel={() => {
