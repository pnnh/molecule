...
import {DeleteButton, RunButton} from './action'
...
export function Table (props: {
  data: SelectResult<PipelineModel>,
  rawQuery: string,
}) {
  ...
  const goPage = (page: number, pageSize: number) => {
    ...
    const redirectUrl = '/console/Pipeline?' + queryString.stringify(queryObject)
...
function TableRow (props: { model: PipelineModel }) {
      ...
      <div className={styles.rowContent + ' row'}>
        ...
        <div className={'col-2 col-xl-2'}>
          <div className={styles.fieldItem}>
            <label>构建参数</label>
            <div className={styles.content}>
              <Link href={'/console/parameter?pipeline=' + props.model.id}>构建参数</Link>
            </div>
          </div>
        </div>
        <div className={'col-2 col-xl-2'}>
          <div className={styles.fieldItem}>
            <label>构建阶段</label>
            <div className={styles.content}>
              <Link href={'/console/stage?pipeline=' + props.model.id}>构建阶段</Link>
            </div>
          </div>
        </div>
        <div className={'col-2 col-xl-2'}>
          <div className={styles.fieldItem}>
            <label>构建历史</label>
            <div className={styles.content}>
              <Link href={'/console/history?pipeline=' + props.model.id}>构建历史</Link>
            </div>
          </div>
        </div>
      </div>
...
function calcOperatorItems (model: PipelineModel): MenuProps {
