...
import {Checkbox, ConfigProvider, MenuProps, Pagination} from 'antd'
...
export function Table (props: {
  ...
  const goPage = (page: number, pageSize: number) => {
    const query = queryObject || new ArtifactSearchModel()
    query.page = page
    query.size = pageSize
    const redirectUrl = '/console/artifact?' + queryString.stringify(query)
...
function TableRow (props: { model: ArtifactDto }) {
    ...
    <div className={styles.dataRow}>
      <div className={styles.rowStart}>
        <div className={styles.rowStartContent}>
          ...
          <div className={styles.modelId}>{props.model.id}</div>
        </div>
      </div>
      <div className={styles.rowContent + ' row'}>
        <div className={styles.dataCell + ' col-2 col-xl-2'}>
          <div className={styles.fieldItem}>
            <label>仓库名称</label>
            <div className={styles.content} title={props.model.name}>
              {props.model.name}</div>
          </div>
        </div>
...
function calcOperatorItems (model: ArtifactDto): MenuProps {
