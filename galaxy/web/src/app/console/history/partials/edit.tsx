...
export function EditButton (props: { model: HistoryModel }) {
  const model = props.model
  const [show, setShow] = React.useState(false)
  const router = useRouter()


  const onFormFinish = async (values: HistoryModel) => {
    console.log('Success:', values)
    const result = await HistoryClient.updateModel(values)
  ...
  return <>
    ...
    <Modal title="编辑仓库" footer={null} open={show} onOk={async () => {
      ...
      >
        ...
        >
          <Input required={true} value={model.title} placeholder={'仓库名称'}/>
        </Form.Item>
        <Form.Item
          label="仓库描述"
          name="description"
