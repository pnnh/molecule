...
export function Table (props: {
  ...
  const goPage = (page: number, pageSize: number) => {
    ...
    const redirectUrl = '/console/Task?' + queryString.stringify(queryObject)
...
function TableRow (props: { model: TaskModel }) {
      ...
      <div className={styles.rowContent + ' row'}>
        ...
        <div className={'col-2 col-xl-2'}>
          <div className={styles.fieldItem}>
            <label>构建任务</label>
            <div className={styles.content}>
              <Link href={'/console/task?Task=' + props.model.id}>构建任务</Link>
            </div>
          </div>
        </div>
      </div>
...
function calcOperatorItems (model: TaskModel): MenuProps {
