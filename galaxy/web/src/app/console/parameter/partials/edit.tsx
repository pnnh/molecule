...
export function EditButton (props: { model: ParameterModel }) {
  const model = props.model
  const [show, setShow] = React.useState(false)
  const router = useRouter()


  const onFormFinish = async (values: ParameterModel) => {
    console.log('Success:', values)
    const result = await ParameterClient.updateModel(values)
