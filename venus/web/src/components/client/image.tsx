...
export function PSImage (props: {src:string, alt?:string, width?:number, height?:number,
    fill?:boolean, sizes?:string}) {
  const alt = props.alt ? props.alt : ''
  const defaultImage = '/images/default.webp'
  const imageUrl = props.src ? props.src : defaultImage
  const [src, setSrc] = useState(imageUrl)
  if (props.fill) {
    return <Image onError={() => setSrc(defaultImage)} src={src} alt={alt}
     fill={props.fill} sizes={props.sizes}></Image>
  }
