...
async function verify (token: string) {
  ...
  console.log('decodeOut', decodeOut)
  if (typeof (decodeOut as jwt.JwtPayload).iss === 'string') {
    ...
    console.debug('id_token验证结果', verifyResult)
...
export async function getAccessToken () {
  ...
  const cacheKey = 'polaris:token:' + decodeToken.jti
...
export async function getIdentity (): Promise<string | undefined> {
  ...
  const cacheKey = 'polaris:identity:' + decodeToken.jti
  ...
  try {
    ...
    console.error('校验id_token出现异常', ex)
    return undefined
  }
