...
export default function Home({searchParams}: {
    ...
    if (serverConfig.NEXT_PUBLIC_SIGN_PASSWORD === 'ON') {
        signMethods.push({
            ...
            title: '账号密码',
            form:
                <PasswordForm serverUrl={serverConfig.NEXT_PUBLIC_SERVER}
                              authServer={serverConfig.NEXT_PUBLIC_AUTH_SERVER} rawQuery={rawQuery}/>
        })
    }
                    ...
                    <div className={styles.titleRight}>
                        {
                            signMethods.map((method) => {
                                return <React.Fragment key={method.key}>
                                    <Link href={''} className={loginMethod === method.key ? 'active' : ''}
                                          onClick={() => {
