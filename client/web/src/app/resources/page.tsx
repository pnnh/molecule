...
import {ApplicationModel, selectPublicApplications} from '@/models/applications'
...
export async function LoadResourceList (page = 1) {
  const pageSize = 8
  const result = await selectPublicApplications(page, pageSize)
...
export function ResourceItem (props: { model: ApplicationModel }) {
  const readUrl = '/users/' + props.model.pk
